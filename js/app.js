...
const LEGAL_VERSION = '2026-02-08-ga4';
const GA_MEASUREMENT_ID = 'G-ZH7W86ZQSP';
    ...
    privacy: {
      ...
      body: `
        ...
        <ul>
          ...
          <li>利用状況の解析のため Google Analytics（GA4）を利用します。同意後に匿名化された計測データ（閲覧ページ、操作イベント等）を送信します。</li>
        </ul>
        <p>利用目的</p>
        <ul>
    ...
    privacy: {
      ...
      body: `
        ...
        <ul>
          ...
          <li>We use Google Analytics (GA4) for usage analytics. After consent, anonymized metrics such as page views and interaction events may be sent.</li>
        </ul>
        <p>Purpose of use</p>
        <ul>
...
function acceptLegalConsent() {
  ...
  if (overlay) overlay.classList.remove('visible');
  enableAnalyticsIfConsented(true);
}

function isLegalConsentGranted() {
  return localStorage.getItem(LEGAL_STORAGE_KEY) === LEGAL_VERSION;
}

function enableAnalyticsIfConsented(forceGrant = false) {
  if (typeof window.gtag !== 'function') return false;
  if (!forceGrant && !isLegalConsentGranted()) return false;
  window.gtag('consent', 'update', { analytics_storage: 'granted' });
  if (!window.__ppPageViewSent) {
...
function trackEvent(eventName, params = {}) {
  ...
  if (!enableAnalyticsIfConsented()) return;
...
function showEndgameOverlay() {
  ...
  const stats = getEndgameStats();
  trackEvent('show_result', {
    mode: appMode,
    round: currentRoundNumber || 0,
    hand_count: stats.handCount || 0
  });
...
function startGame() {
  ...
  trackEvent('start_game', {
    ...
    small_blind: settings.smallBlind || 0,
    big_blind: settings.bigBlind || 0
  });
