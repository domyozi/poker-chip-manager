...
## ✅ 完了
- ✅ 非アクティブなターン停止を回避（フォールド周りの自動補正）
- ✅ 全員フォールド時に自動で勝利表示へ遷移
- ✅ 次のハンド表示に順位とスタック/変動形式を追加
- ✅ 履歴の表示形式を時刻付き・スタック/変動形式に更新
- ✅ アクション/ステータス表示のUI調整（サイズ・表示簡略）
- ✅ トーナメントモードをオンライン限定で復元
- ✅ 設定画面の効果音位置/プレイヤー操作UIを調整
- ✅ トーナメントモードを削除（ロードマップへ移動）
- ✅ メニューからチップ調整/プレイヤー追加・退席が可能
- ✅ ショーダウンでメイン/サイドポット別の勝者選択タブを追加
- ✅ MIN RAISE未満でもALL INできるアクション導線を追加
- ✅ オフライン/オンラインのモード管理をappModeで統一（リーク対策）
- ✅ デバッグバナー追加（DEBUG_MODE or localStorage debugで表示）
- ✅ モードリーク回帰テストケース追加（TEST_MATRIX_2P.md）
- ✅ オフライン開始時の「—のターン」表示を抑止（ローカル判定/フォールバック追加）
- ✅ オフライン開始時にアクションが出ない問題を修正（ローカル扱いの判定を強化）
...
## 🧭 次の予定
- P0: サイドポットの概念を導入（オールイン時のポット分割）
- P1: 縦長（縦持ち）UIへのレイアウト変更（添付イメージに寄せる）
- P1: フォールド時に「手札を前へフリック」でフォールドできる直感的UI
- P1: 複数チップの擬似チップ管理UI（複数チップでのベット表現/操作）
...
- P1: 隠しGTOモード（トグルUIは非表示、特定操作で有効化）
- P1: 初心者向けプリフロップレンジ表を表示（用語/行動の簡易解説つき）
...
## リリースノート
### v0.8.12
- テーブル周辺UIの再配置（縦テーブル対応・POT位置調整）
- ベットチップ表示を固定マーカー化（table-bets内）
- 勝利後の表示をPOT総額表記に統一
- オフライン時の接続インジケータ非表示 / 音量トグル復帰 / 履歴アバター縮小

### v0.8.1
- オフライン/オンライン切り替えのモードリーク対策（appMode導入）
- オフライン移行のリセット処理を専用化
- デバッグ用モードバナー（フラグ有効時のみ表示）
